...
from .models import (
    ...
    ParticipantRole, BusinessEntityType
)
...
class ContractForm(forms.ModelForm):
    cv_code = forms.CharField(
        required=False,
        label='CV Code',
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'e.g., CV-2026-001'}),
        help_text='Required for CV business entity and will be carried to structured data automatically.'
    )
    ...
    def __init__(self, *args, **kwargs):
        self.enforce_auto_period = kwargs.pop('enforce_auto_period', False)
        initial_cv_code = kwargs.pop('initial_cv_code', '')
        ...
        entity_type = self.initial.get('business_entity_type') or getattr(self.instance, 'business_entity_type', None)
        if entity_type == BusinessEntityType.CV:
            self.fields['cv_code'].required = True
            self.fields['cv_code'].initial = initial_cv_code or self.initial.get('cv_code', '')
        else:
            self.fields.pop('cv_code', None)

        if self.enforce_auto_period:
            self.fields['end_date'].widget.attrs['disabled'] = 'disabled'
            self.fields['end_date'].help_text = 'Calculated automatically from start date + duration (days).'

            # For legacy records, infer duration so existing dates remain visible/editable.
            if (
                not self.is_bound
                and getattr(self.instance, 'start_date', None)
                and getattr(self.instance, 'end_date', None)
                and not getattr(self.instance, 'duration_days', None)
            ):
                delta_days = (self.instance.end_date - self.instance.start_date).days
                if delta_days > 0:
                    self.initial.setdefault('duration_days', delta_days)
                    self.fields['duration_days'].initial = delta_days
    ...
    def clean(self):
        ...
        if self.enforce_auto_period:
            ...
            if duration_days in (None, ''):
                self.add_error('duration_days', 'Contract duration in days is required.')
            elif duration_days <= 0:
                self.add_error('duration_days', 'Duration must be greater than 0 days.')

            if start_date and duration_days and duration_days > 0:
                cleaned_data['end_date'] = start_date + timedelta(days=duration_days)
...
class ContractStatusUpdateForm(forms.ModelForm):
    ...
    def __init__(self, *args, user=None, contract=None, **kwargs):
        ...
        if not _supports_contract_number():
            self.fields.pop('contract_number_override', None)
            return
        ...
        can_override_contract_number = (
            not getattr(target_contract, 'contract_number', '')
    ...
    def clean(self):
        ...
        if self.instance and getattr(self.instance, 'contract_number', ''):
            self.add_error(
                'contract_number_override',
        ...
        if not _supports_contract_number():
            self.add_error(
                'contract_number_override',
                'Contract number feature is not available in this deployment.'
            )
            return cleaned_data
...
class ContractNumberOverrideForm(forms.Form):
    ...
    def clean_contract_number(self):
        ...
        if not _supports_contract_number():
            raise forms.ValidationError('Contract number feature is not available in this deployment.')
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['status'].choices = [
            ...
            *[
                (value, label)
                for value, label in ContractStatus.choices
                if value != ContractStatus.SUBMITTED
            ],
        ]
