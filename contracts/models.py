...
class ContractTypeDefinition(models.Model):
    ...
    number_prefix = models.CharField(
        ...
        blank=True,
        default='',
        help_text="Contract number prefix for this type (e.g. 01, 02, 03). Leave blank to disable auto-numbering."
    )
    number_company_code = models.CharField(
        max_length=20,
        blank=True,
        default='PCI',
        help_text="Company segment in contract number format. Example: PCI"
    )
    number_department_code = models.CharField(
        max_length=20,
        blank=True,
        default='SALES',
        help_text="Department segment in contract number format. Example: SALES"
    )
    product_scope_text = models.TextField(
        blank=True,
        default='',
        help_text="Configurable product scope text for templates (available as {{ product_scope_text }})."
    )
    incentive_scheme_text = models.TextField(
        blank=True,
        default='',
        help_text="Configurable incentive scheme text for templates (available as {{ incentive_scheme_text }})."
    )
...
class ContractNumberSequence(models.Model):
    """Monthly sequence counter per contract type for contract numbering."""
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['contract_type', 'year', 'month']),
        ]

    def __str__(self):
        return f"{self.contract_type.code} {self.year}-{self.month:02d} #{self.last_number}"


class Contract(models.Model):
    # Basic Information
    title = models.CharField(max_length=255)
    contract_number = models.CharField(max_length=64, null=True, blank=True, unique=True)
