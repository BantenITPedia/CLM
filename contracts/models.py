...
class Contract(models.Model):
    ...
    contract_number = models.CharField(
        ...
        null=True,
        blank=True,
        help_text='Generated at legal approval (e.g., 01.001/PCI/Sales/2026)'
    )
    description = models.TextField(blank=True)
    ...
    issuing_company_name = models.CharField(
        max_length=255,
        null=True,
        blank=True,
        help_text='Issuing company name (required for Vendor contracts)'
    )
    issuing_company_code = models.CharField(
        max_length=50,
        null=True,
        blank=True,
        help_text='Issuing company code used in contract number (e.g., PCI)'
    )
...
class ContractNumberSequence(models.Model):
    """Track running numbers for each contract type/year/company combination"""

    contract_type_code = models.CharField(max_length=2, help_text='01, 02, 03, 04')
    year = models.PositiveIntegerField()
    company_key = models.CharField(
        max_length=100,
        null=True,
        blank=True,
        help_text='Company key for Vendor (04). Null for non-vendor types.'
    )
    ...
    def __str__(self):
        company_label = self.company_key or 'GLOBAL'
        return f"{self.contract_type_code}/{self.year}/{company_label}: {self.last_number}"
